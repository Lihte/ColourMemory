...
public class ConsoleUI
{
    ...
    public void PrintGameLoop()
    {
        ...
        while (playing)
        {
            while (!_game.AllPairsFound())
            {
                ...
                var first = FlipCard("Select first card");

                PrintGameBoard();

                var second = FlipCard("Select second card", first);
    ...
    private void PromptSaveHighScore()
    {
        ...
        HighScoreSerializer.SaveHighScore(_game.Score, input, _game.Difficulty);
    }
    ...
    private void PrintGameBoard()
    {
        ...
        Console.WriteLine("\n    0   1   2   3");
        for (int i = 0; i < _game.GameBoard.Rows; i++)
        {
            Console.Write($"{i}  ");
            for (int j = 0; j < _game.GameBoard.Columns; j++)
            {
                if (_game.GameBoard.GetCard(i, j).IsPaired)
                {
                    Console.Write("    ");
                }
                else if (_game.GameBoard.GetCard(i, j).IsViewed)
                {
                    Console.Write($"[{_game.GameBoard.GetCard(i, j).Colour}] ");
                }
                else
                    Console.Write("[X] ");
    ...
    /// </summary>
    private (int, int) FlipCard(string prompt, (int, int)? isFlipped = null)
    {
        int row, col;
        ...
        while (true)
        {
            ...
            Console.Write($"Input row (0 - {_game.GameBoard.Rows - 1}): ");
            if (!int.TryParse(Console.ReadLine(), out row) || row < 0 || row > _game.GameBoard.Rows - 1)
            {
            ...
            if (!int.TryParse(Console.ReadLine(), out col) || col < 0 || col > _game.GameBoard.Columns - 1)
            {
